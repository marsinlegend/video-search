...
const ImageWrapper = styled.div`
    display: grid;
    grid-template-columns: repeat(${({ video, viewMethod }) => (video === 1 ? 3 / viewMethod : 6 / viewMethod)}, 1fr);
    ...
    <div>
      <Container>
        <ContainLeft>
          <SearchContainer>
            ...
            <IconWrapper><Devices /></IconWrapper>
          </SearchContainer>
          <OnlineSelect>
            ...
            <span><FormattedMessage {...messages.onlineState} /></span>
          </OnlineSelect>
        </ContainLeft>
        <ContainRight>
          <ViewWrapper>
            <FormattedMessage {...messages.view} /> &nbsp;&nbsp;&nbsp;
            <CardWrapper view={viewMethod === 1} onClick={() => { setView(1); }}><SmallCard /></CardWrapper>
            <CardWrapper onClick={() => { setView(2); }}><MidCard /></CardWrapper>
            <CardWrapper><BigCard /></CardWrapper>
            <CardWrapper><OneCard /></CardWrapper>
            <WallBtn><FormattedMessage {...messages.customWall} /></WallBtn>
            <SortWrapper>
      ...
      <Content>
            ...
            <ImageWrapper video={video} viewMethod={viewMethod} ref={scrollRef}>
              {MockData.map((Mock, id) => (
                <ImageW key={`mock-video-${id}`}>
                  ...
                  <Rectangle width={`${285 * viewMethod}px`} height={`${173 * viewMethod}px`} videoId={videoId === id} onClick={() => {
                    setVideo(1);
        ...
        <VideoWrapper video={video}>
          <CameraInfo>
            <MyArrow onClick={() => { setVideo(0); setVideoId(""); }} ><Arrow /></MyArrow>
            <MySpan>{roomId + " - " + cameraId}</MySpan>
            <Ellipse />
          </CameraInfo>
          <SearchBtn><Search />&nbsp;&nbsp;&nbsp;<FormattedMessage {...messages.searchRecording} /></SearchBtn>
          <div>
            <ReactPlayer url="../../Media/test.mp4" controls={true} width="100%" />
          </div>
          <ClipBtn><Archieve />&nbsp;&nbsp;&nbsp;<FormattedMessage {...messages.makeClip} /></ClipBtn>
        </VideoWrapper>
      </Content>
    </div>
  )
